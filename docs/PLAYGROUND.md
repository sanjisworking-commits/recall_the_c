...
| Document | Status |
|----------|--------|
| [PLAYGROUND_DELIVERY_TRACKER.md](PLAYGROUND_DELIVERY_TRACKER.md) | **Programme scoreboard.** Stage 1 = **48.6 / 100** (build). Milestone 2 is `DONE` (37/37). Milestone 3 is `DONE` (15/15). Milestone 4 is `DONE` (25/25). Milestone 5 is `IN PROGRESS` (17/27, M5-A). Stage 2 optimize is `NOT STARTED` until Stage 1 is `DONE` (100/100). Cloze/overlay proof does **not** inflate either stage. |
...
| [PAYMENT_ENTITLEMENT_AUDIT.md](PAYMENT_ENTITLEMENT_AUDIT.md) | User-type + **device registry** + **monthly roster** (`Asia/Kolkata` period clock). Catalogue, `user_subscription` persistence, M2 lifecycle, M3-A `EntitlementService` (Constitution inversion), M3-B Playground commercial HTTP gate, M4-A two-device authorization, M4-B owner/admin device management, M4-C replacement-churn, and **M5-A current-period roster** exist. Platforms are `web | android | ios`. Replacement-churn rate policy is locked in §21 (`DEVICE_REPLACEMENT_WINDOW_DAYS = 30`, `DEVICE_REPLACEMENT_LIMIT = 3`, support email via `SUPPORT_EMAIL`). Overlay is lifetime learning; roster is current-month membership (`overlay != roster`). Pending + current-roster law is learnable; pending + historical overlay only is not. M5-B rollover remains open. Commercial catalogue, access matrix, refunds, and disputes are **locked** in §21. |
