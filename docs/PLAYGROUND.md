...
| Document | Status |
|----------|--------|
| [PLAYGROUND_DELIVERY_TRACKER.md](PLAYGROUND_DELIVERY_TRACKER.md) | **Programme scoreboard.** Stage 1 = **40.7 / 100** (build). Milestone 2 is `DONE` (37/37). Milestone 3 is `DONE` (15/15). Milestone 4 is `IN PROGRESS` (24/25, M4-A+M4-B). Stage 2 optimize is `NOT STARTED` until Stage 1 is `DONE` (100/100). Cloze/overlay proof does **not** inflate either stage. |
...
| [PAYMENT_ENTITLEMENT_AUDIT.md](PAYMENT_ENTITLEMENT_AUDIT.md) | User-type + **device registry** + **monthly roster** (`Asia/Kolkata` period clock). Catalogue, `user_subscription` persistence, M2 lifecycle, M3-A `EntitlementService` (Constitution inversion), M3-B Playground commercial HTTP gate, M4-A two-device authorization, and M4-B owner/admin device management exist. Platforms are `web | android | ios`. Replacement-churn rate policy is BLOCKED. Monthly roster enforcement is Milestone 5. Commercial catalogue, access matrix, refunds, and disputes are **locked** in §21. Remaining open cells: `DEVICE_REPLACEMENT_WINDOW_DAYS`, `DEVICE_REPLACEMENT_LIMIT`, support contact channel. |
