...
**Scope of this document.** This is the **locked product architecture** for RecallC access: user types, device control for paid Playground, monthly Playground roster, clocks, schema to build, CTA states, sequential batches, the locked commercial catalogue / subscription-access matrix, and locked device-churn cells. It is **not** a description of current production behaviour. Current code still uses Razorpay **one-time duration passes** as historical Constitution commerce. Playground has an additive `user_subscription` table, Plus/Pro/Max catalogue, M2 lifecycle, M3-A [`EntitlementService`](../src/constitution_memorizer/entitlements/service.py) that inverts Constitution (authenticated = full Learn), an M3-B commercial HTTP gate on `/playground` via [`playground/access.py`](../src/constitution_memorizer/playground/access.py), and **M4 device authorization + management + replacement-churn** in [`devices/`](../src/constitution_memorizer/devices/) (`user_device` / `user_device_session` / `user_device_replacement`, cookie `rtc_device`, cap 2 for every paid tier, platforms `web | android | ios`, `GET /profile/security/devices`, audited admin reset, rolling 3/30 replacement limit, configurable `SUPPORT_EMAIL`). Device registry, management, and replacement-churn policy are implemented. **M5 monthly roster and rollover are implemented** (`user_playground_period` / `user_playground_roster_item`, Asia/Kolkata, [`playground/roster/`](../src/constitution_memorizer/playground/roster/)). Overlay items are persistent learning, not current-month roster membership (`overlay != roster`). **Locked pending rule:** pending + law active on the **current roster** → learnable; pending + historical overlay only → not learnable / cannot consume. Carry-forward candidates come only from the immediately previous Asia/Kolkata month. Keep consumes that target period. Decline does not. Saved progress never waives the new month's slot. The overlay is a Cloze **proof**, not the finished product.

**Implementation note (M6 production UX).** Milestone 6 is presentation/read-model integration against the locked M1–M5 contracts. Production uses [`docs/design/Playground.dc.html`](design/Playground.dc.html) as the visual source and [`playground/view.py`](../src/constitution_memorizer/playground/view.py) as the mapper. Preview controls, `support.js`, and hardcoded prototype laws are **not** runtime. Guest Playground remains Sign in (never checkout). Pending + current-roster learning stays allowed; pending + new/historical consume is blocked; same-month Add back is allowed. Undecided rollover candidates consume 0 and are not auto-carried. Device, payment, and quota authorization are unchanged.
...
**Implementation note (M2-B provider bound / M2-C webhooks / M2-D lifecycle).** Razorpay requires every subscription to be bounded. Create calls use `RAZORPAY_MONTHLY_TOTAL_COUNT = 1200` monthly cycles as a *provider-only* technical horizon — not a 100-year customer commitment, not an annual SKU, and not copy shown to users. RecallC remains monthly cancel-anytime; default user cancel is `cancel_at_cycle_end=true`. Scheduled downgrade targets are stored in `provider_metadata` (`scheduled_tier`, `scheduled_plan_id`, `schedule_change_at`) and become the paid tier only when a later provider GET confirms the new plan id. Subscription webhooks verify HMAC over the **raw request body**, persist `x-razorpay-event-id`, and reconcile from `GET /v1/subscriptions/{id}` — they are not themselves authorization truth. Failed auto-charge persists provider `pending`; exhausted retries persist `halted`; both stay current and recoverable. Provider-confirmed `cancelled` / `completed` / `expired` archive the row (`is_current=false`) so resubscribe creates a **new** provider subscription. Refunds and disputes resolve payment → invoice → `subscription_charge` (never email/amount matching); `access_effect` is computed from current provider refund/dispute status. Legacy N-day buyers are classified from `access_grants.source='payment'` plus optional `billing_orders` via `reason='razorpay:{order_id}'` — they are not migrated into `user_subscription` and are not Plus/Pro/Max. M3-A `EntitlementService` consumes those facts for Playground *commercial* snapshot fields; `/playground` HTTP gating is Milestone 3B.
...
Constitution Learn capability is resolved in [`src/constitution_memorizer/web/entitlements.py`](../src/constitution_memorizer/web/entitlements.py) as an adapter around [`entitlements/`](../src/constitution_memorizer/entitlements/):

| Code | Meaning after M3-A |
|------|---------|
| `guest` | Multiuser on, no `current_user` — explore (flag-on: four open modes) |
| `subscribed` (capability label) | Any signed-in account, or local single-user owner — full Constitution. **Not** a Playground purchase. |
...
Flag: `ARTICLE_ENTITLEMENTS_ENABLED` → `entitlements_active()`. **Default false.** After M3-A the flag is **partially obsolete**: it no longer restores the 3-Article cap or Type/Recite premium lock for **signed-in** accounts. Authenticated Learn is always full Constitution (all Articles, all six modes, persistent Done / `modes_seen`, no claim prompt). The flag still controls **guest** explore (four open modes when on; legacy full guest access when off) and historical status-surface rendering.
...
| Signed-in account (any payment state) | All 6 modes, all Articles, persist; claims / slot cap / premium-mode names are not authorization input |
| Admin Entitlement Preview | Still synthesizes the historical free/guest matrix without writing progress |

`FREE_ARTICLE_LIMIT = 3`. Table `user_free_articles` is retained as history. Grandfathering still exists on `claimed_articles()`; normal authenticated Done does not insert a new claim.

**Laws / Bare Acts:** not on this matrix. Feature flag `RELEVANT_LAWS_ENABLED` 404s `/laws*` only. Reading is free.

**Playground (existing overlay on `cursor/playground-220d`):** M3-B commercially gates `/playground` from `EntitlementSnapshot` (`can_open_playground` vs `can_consume_new_playground_law`). Guests Sign in; signed-in free/legacy Subscribe; halted/paused/paid-period-ended use payment-state copy; pending + law active on the **current roster** may continue Learn; pending + historical overlay only is not learnable and cannot consume a new law. Same-period re-add of an already-consumed removed law is not new consumption. Active and admin keep six-mode Playground learning **and** require current-roster membership. Overlay rows are **persistent learning**, not a monthly roster and not a device registry (`overlay != roster`). **M3 commercial gate is complete.** **M4-A core two-device authorization is shipped:** paid Playground registers HMAC-hashed `rtc_device` installations (limit 2, all tiers, `web | android | ios`) on first eligible use; a third or revoked installation is Playground-blocked with `device_limit` / `device_revoked` while `is_subscribed` stays true and Constitution / `/laws` stay open. **M4-B owner/admin management is shipped** (`GET /profile/security/devices`, current-device HMAC label, owner revoke, device-limit/revoked gates, audited `POST /admin/users/{user_id}/devices/reset`). **M4-C replacement-churn is shipped:** at most 3 counted replacements in a rolling 30-day UTC window; a fourth attempt returns `playground_block_reason=device_replacement_limit` without banning, cancelling, or deleting progress; existing active devices remain usable; support CTA is `mailto:` only when `SUPPORT_EMAIL` is configured; admin `reset_devices` does not erase replacement history; audited `POST /admin/users/{user_id}/devices/clear-replacement-limit` clears the rolling lock. Monthly roster enforcement remains M5. Do not count overlay rows against 10/30/unlimited.
...
**There is no `subscriptions` table.** **`user_device` / `user_device_session` / `user_device_replacement` exist** (M4-A, [`20260916_0021_devices.py`](../alembic/versions/20260916_0021_devices.py); iOS platform CHECK in [`20260917_0022_device_ios_platform.py`](../alembic/versions/20260917_0022_device_ios_platform.py); replacement events in [`20260917_0023_device_replacement.py`](../alembic/versions/20260917_0023_device_replacement.py)). **M5-A roster tables exist** ([`20260917_0024_playground_roster.py`](../alembic/versions/20260917_0024_playground_roster.py)): `user_playground_period` `UNIQUE(user_id, period_start)` and `user_playground_roster_item` `UNIQUE(user_id, period_start, law_id)` — not a lifetime `UNIQUE(user_id, law_id)`. Do **not** implement `user_playground_law_entitlement`. Destination rollover uses the same tables (`carry_forward`, `declined_at`, draft periods, `GET/POST /playground/roster/next`). Previous-month laws are candidates only. §21 device-churn cells are **locked**: `DEVICE_REPLACEMENT_WINDOW_DAYS = 30`, `DEVICE_REPLACEMENT_LIMIT = 3`, support channel = email via `SUPPORT_EMAIL`.
...
| Playground overlay | [`20260906_0017_playground_overlay.py`](../alembic/versions/20260906_0017_playground_overlay.py) | `user_playground_item` / `selection` / `progress` — **persistent learning only** (`overlay != roster`). Lifecycle/schedule rows (`learned` / `review` / `mastered`, `learned_at`, `next_revision`) are M8. |
| Playground mode progress | [`20260924_0025_playground_mode_progress.py`](../alembic/versions/20260924_0025_playground_mode_progress.py) | `user_playground_mode_progress` — independent Read/Cloze/Letters/Type/Recite/Test **initial** completion. Type is write-it-out. No seventh Write mode. Lifetime evidence; not reset by later revisions. |
| Playground revision lifecycle | [`20260925_0026_playground_revision_lifecycle.py`](../alembic/versions/20260925_0026_playground_revision_lifecycle.py) | `user_playground_progress.learned_at`; `user_playground_revision_mode_progress` per-rung six-mode cycles; due index `(user_id, next_revision, status)`. Does not invalidate on statute change (M9). |
| Playground roster | [`20260917_0024_playground_roster.py`](../alembic/versions/20260917_0024_playground_roster.py) | `user_playground_period` + `user_playground_roster_item` — current Asia/Kolkata month membership. Not a lifetime entitlement. |
...
| Mechanism | Today |
|-----------|--------|
| Pass expiry | `access_grants.ends_at`; store query drops expired grants; user keeps full Constitution because they are authenticated. Playground snapshot `legacy_status` becomes `legacy_expired`. |
...
**Do not** store roster usage, `playground_period_*`, or device registrations on this row. Index `(status, billing_period_end)` exists for later expiry/retry work. M2-B create/checkout/cancel/plan-change persist into this table. M2-C webhooks update the same row from provider GET (renewal bounds, confirmed plan changes). M2-D archives provider-confirmed `cancelled` / `completed` / `expired` rows (`is_current=false`) so a later resubscribe can insert a new current row; `pending` / `halted` / `paused` stay current. Entitlement inversion is **not** in this batch. A scheduled downgrade target may live in `provider_metadata` until a later provider GET confirms the new plan.

### `subscription_webhook_event` (M2-C)

Provider event-id reservations for idempotent webhook processing. **Not** a user-owned entitlement table. Unique on `(provider, provider_event_id)`. Stores event name, optional provider subscription id, payload SHA-256, and processing status (`processing` / `processed` / `ignored` / `failed` / `unmatched`). Does **not** store raw payment/card payloads, signatures, or webhook secrets. RLS enabled with no policies (application-level isolation). Refund and dispute deliveries reuse this table.

### `subscription_charge` (M2-D)

One Razorpay recurring payment, unique on `(provider, provider_payment_id)`. Linked through payment id → invoice id → provider subscription id → `user_subscription` — never email, phone, amount, or tier-price matching. Stores billing window, payment/refund/dispute identifiers and statuses, and a deterministic `access_effect` (`none` / `period_ended`) with reason `full_refund` or `dispute_lost`. A full refund or lost dispute ends **that paid period only**; the parent subscription may remain `active`. Does **not** store card, contact, email, VPA, raw webhook, or signature data.
...
| **E — Learning** | All six RecallC-style modes on Bare Acts; Learned only after all six; then Day 1 → 3 → 7 → 15 → 30 → 60 → Mastered. Overdue advances one rung. | Overlay progress schema extended (`learned_at`, revision-mode table). **Do not** use progress rows as quota or as device identity. URLs under `/playground/laws/{law_id}/…` |
